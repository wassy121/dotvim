...
augroup perforce
  ...
  map <silent> <Leader>4u :echo <SID>P4GetUser()<CR>
  map <silent> <Leader>4d :echo <SID>P4UDiffFile()<CR>
...
"----------------------------------------------------------------------------
" Get the current p4 user and client from set
"----------------------------------------------------------------------------
function s:P4GetUser()
  let user = ''
  let pout = s:P4ShellCommand("set" . listnum)
  if v:errmsg != ""
    echoerr "Unable to get variables. " . v:errmsg
    return ''
  endif
  return pout
  let options = split(pout, "\n")
  "for optionitem in options
  "  if matchstr( optionitem, "P4USER" )
  "    continue
  "  endif
  "   let user = optionitem
  " endfor
...
function s:P4GetChangelists(sAll)
  let opt = ''
  if (a:sAll == 0)
    let opt = ' -c ' . "samuel.stringham_SSTRINGHAM-NBK"
  endif
  let cmd = "changes -L -s pending -u samuel.stringham " . opt
